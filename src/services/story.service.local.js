...
async function query(filterBy = { txt: "", tag: "" }) {
  ...
  if (!stories || stories.length === 0) {
    ...
    stories = demoStoryService.generateRandomStories(50);
  ...
  if (filterBy.txt) {
    ...
    stories = stories.filter(
      (story) => regex.test(story.txt) || regex.test(story.description)
    );
  }
  if (filterBy.tag) {
    stories = stories.filter((story) =>
      story.tags.some((tag) => tag === filterBy.tag)
    );
  }
  ...
  if (filterBy.likedById) {
    stories = stories.filter((story) =>
      story.likedBy.some((miniUser) => miniUser._id === filterBy.likedById)
    );
  }
  return stories;
}
